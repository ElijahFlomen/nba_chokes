...
home_chokes_plot <- ggplot(home_choke_reason_max) + geom_bar(aes(home_mistake), color='#56545c', fill='#666a8e') + 
  labs(x="Home Team Mistake", y="Count", title="Home Chokes")
home_chokes_plot

# GRAPH 4: Leading Causes for an away-team choke
away_chokes_plot <- ggplot(away_choke_reason_max) + geom_bar(aes(away_mistake), color='#6084c6', fill="#0fa3ff") + 
  labs(x="Away Team Mistake", y="Count", title="Away Chokes")
away_chokes_plot
```
...
```{r}
okc_player_stats <- 
  players_careers(players=c("Russell Westbrook", "Kenrich Williams", "Dennis Schroder", "Paul George", "Shai Gilgeous-Alexander"),
                  modes=c("Totals"), 
                  assign_to_environment = FALSE,
                  add_mode_names = FALSE, return_message = FALSE) %>% filter(nameTable=="SeasonRankingsRegularSeason")
...
williams_stats <- okc_player_stats %>% filter(namePlayer=="Kenrich Williams") %>% select(dataTable)
williams_stats <- williams_stats[[1]][[1]] %>% select(fg3mRank, fgmRank, pctFGRank, ptsRank, tovRank, trebRank)
williams_stats$player <- "Kenrich Williams"
williams_longer <- williams_stats %>% pivot_longer(!player, names_to = 'stat', values_to = 'value') 
williams_longer <- williams_longer %>% group_by(stat) %>% summarize(mean(value, na.rm=TRUE))
williams_longer$player <- "Kenrich Williams"
...
george_stats <- dort_stats[[1]][[1]] %>% select(fg3mRank, fgmRank, pctFGRank, ptsRank, tovRank, trebRank)
george_stats$player <- "Paul George"
george_longer <- dort_stats %>% pivot_longer(!player, names_to = 'stat', values_to = 'value') 
george_longer <- dort_longer %>% group_by(stat) %>% summarize(mean(value, na.rm=TRUE))
george_longer$player <- "Paul George"

shai_stats <- okc_player_stats %>% filter(namePlayer=="Shai Gilgeous-Alexander") %>% select(dataTable)
shai_stats <- horford_stats[[1]][[1]] %>% select(fg3mRank, fgmRank, pctFGRank, ptsRank, tovRank, trebRank)
shai_stats$player <- "Shai Gilgeous-Alexander"
shai_longer <- horford_stats %>% pivot_longer(!player, names_to = 'stat', values_to = 'value') 
shai_longer <- horford_longer %>% group_by(stat) %>% summarize(mean(value, na.rm=TRUE))
shai_longer$player <- "Shai Gilgeous-Alexander"

okc_total_heat <- rbind(westbrook_longer, adams_longer, schroder_longer, dort_longer, horford_longer) 
colnames(okc_total_heat)[2] <- "rank"

okc_starts_map<-ggplot(data=okc_total_heat, aes(player, stat, fill=rank)) + geom_tile() + 
  labs(x="Player", y="Statistic", title="Player Stats for OKC")
okc_starts_map
```
...
okc_split_data <- okc_split_data %>% filter(str_detect(player, 'Westbrook|Schroder|Gilgeous-Alexander|George|Williams')) 
