...
```{r}
player_split_data <- read.csv("/Users/elijahflomen/Desktop/nba-chokes-testing/data/player_scoring_data.csv")
...
Interestingly enough, it can be seen here that there is no true distinction between across individuals players performances on OKC. During comebacks, players OKC, generally perform relatively consistently, before and after their games' turning point. With the exception of Adams, whose graph appears distorted due to a long strain of injuries he sustained, the players 2 points curves follow relatively similar trends. When compared with the players of MEM during their choke games, the OKC players during their comback games perform fairly consitently throughout the game. As a result, the true nature of a comeback team possibly comes as an entire team effort. 
