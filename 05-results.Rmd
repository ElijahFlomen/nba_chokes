...
```{r}
okc_player_stats <- 
  players_careers(players=c("Russell Westbrook", "Steven Adams", "Dennis Schroder", "Luguentz Dort", "Al Horford"),
                  modes=c("Totals"), 
                  assign_to_environment = FALSE,
                  add_mode_names = FALSE, return_message = FALSE) %>% filter(nameTable=="SeasonRankingsRegularSeason")
...
dort_stats <- okc_player_stats %>% filter(namePlayer=="Luguentz Dort") %>% select(dataTable)
dort_stats <- dort_stats[[1]][[1]] %>% select(fg3mRank, fgmRank, pctFGRank, ptsRank, tovRank, trebRank)
dort_stats$player <- "Luguentz Dort"
dort_longer <- dort_stats %>% pivot_longer(!player, names_to = 'stat', values_to = 'value') 
dort_longer <- dort_longer %>% group_by(stat) %>% summarize(mean(value, na.rm=TRUE))
dort_longer$player <- "Luguentz Dort"

horford_stats <- okc_player_stats %>% filter(namePlayer=="Al Horford") %>% select(dataTable)
horford_stats <- horford_stats[[1]][[1]] %>% select(fg3mRank, fgmRank, pctFGRank, ptsRank, tovRank, trebRank)
horford_stats$player <- "Al Horford"
horford_longer <- horford_stats %>% pivot_longer(!player, names_to = 'stat', values_to = 'value') 
horford_longer <- horford_longer %>% group_by(stat) %>% summarize(mean(value, na.rm=TRUE))
horford_longer$player <- "Al Horford"
