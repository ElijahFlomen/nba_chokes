...
home_chokes_plot <- ggplot(home_choke_reason_max) + geom_bar(aes(home_mistake), color='#56545c', fill='#666a8e') + 
  labs(x="Home Team Mistake", y="Count")
home_chokes_plot
...
away_chokes_plot <- ggplot(away_choke_reason_max) + geom_bar(aes(away_mistake), color='#6084c6', fill="#0fa3ff") + 
  labs(x="Away Team Mistake", y="Count")
away_chokes_plot
```
...
mem_starts_map<-ggplot(data=mem_total_heat, aes(player, stat, fill=rank)) + geom_tile() + labs(x="Player", y="Statistic")
mem_starts_map
...
all_players_split <- 
  ...
  labs(x="Points Scored", y="Density", color="Legend") + 
